...
export function DashboardStatModal({ stat, onClose }: DashboardStatModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div 
                className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            ...
            >
                <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-semibold text-slate-900 text-lg">{stat.name} Details</h3>
                            <p className="text-slate-500 text-sm">Deep dive into your {stat.name.toLowerCase()} metrics</p>
                        </div>
                    </div>
                ...
                <div className="p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                            <p className="text-sm text-slate-500 font-medium">Period Change</p>
                    ...
                    <div className="border border-slate-200 rounded-xl p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h4 className="font-semibold text-slate-900">Historical Trend</h4>
                            <select className="text-sm border-none bg-slate-50 rounded-lg px-3 py-1 text-slate-600 outline-none cursor-pointer hover:bg-slate-100">
                        ...
                        <div className="h-64 w-full">
                            {stat.chartData && stat.chartData.length > 0 ? (
                                ...
                                <div className="flex items-center justify-center h-full bg-slate-50/50 rounded-lg border border-dashed border-slate-200">
                                    <div className="text-center">
                                        <BarChart3 className="w-8 h-8 text-slate-300 mx-auto mb-2" />
                                        <p className="text-slate-400 text-sm">No data available for {stat.name}</p>
                                    </div>
                                </div>
                            )}
                        </div>
